...
def update(metadata):
    ...
    :type metadata: Movie
    """
    if not metadata.id.startswith('caribpr-'):
        return
    ...
    # fill in information
    metadata.title = "CARIBPR-{}{}".format(item.id, part_text)
    ...
    if image_helper.does_image_exist(item.poster_url):
    ...
    poster = image_helper.add_padding_to_image_as_poster(poster_url)

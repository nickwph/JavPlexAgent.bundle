...
from service.fanza import service_fanza_api
from plex_log import Log

environments.is_local_debugging = True  # this is needed
reload(service_fanza_api)


class Test(TestCase):

    def test_parse_as_dvd_product_id(self):
        self.assertEqual("ssni558", service_fanza_api.parse_as_dvd_product_id("SSNI-558"))
        self.assertEqual("ssni558", service_fanza_api.parse_as_dvd_product_id("SSNI558"))
        self.assertEqual("ssni558", service_fanza_api.parse_as_dvd_product_id("ssni558"))
        self.assertEqual("ssni558", service_fanza_api.parse_as_dvd_product_id("   SSNI-558 "))

    def test_parse_as_digital_product_id(self):
        self.assertEqual("ssni00558", service_fanza_api.parse_as_digital_product_id("SSNI-558"))
        self.assertEqual("ssni00558", service_fanza_api.parse_as_digital_product_id("  SSNI-558 "))
        self.assertEqual("ssni558", service_fanza_api.parse_as_digital_product_id("SSNI558"))

    def test_search_dvd_product___one_result(self):
        body = service_fanza_api.search_dvd_product("SSNI-558")
    ...
    def test_search_dvd_product___one_result_2(self):
        body = service_fanza_api.search_dvd_product("SOAV062")
        self.assertEqual(200, body.result.status)
        self.assertEqual(1, body.result.total_count)
    ...
    def test_search_dvd_product___no_result_for_vr_product(self):
        body = service_fanza_api.search_dvd_product("SIVR-002")
        self.assertEqual(200, body.result.status)
        self.assertEqual(0, body.result.total_count)
        Log.Debug(u"Body returned: {}".format(json.dumps(munchify(body), indent=2, ensure_ascii=False)))

    def test_search_digital_product___one_result(self):
        body = service_fanza_api.search_digital_product("SSNI-558")
        self.assertEqual(200, body.result.status)
        self.assertEqual(1, body.result.total_count)
    ...
    def test_search_digital_product___two_result(self):
        body = service_fanza_api.search_digital_product("SIVR-002")
        self.assertEqual(200, body.result.status)
    ...
    def test_get_dvd_product(self):
        body = service_fanza_api.get_dvd_product("ssni558")
        self.assertEqual(200, body.result.status)
        self.assertEqual(1, body.result.total_count)
    ...
    def test_get_dvd_product___auto_convert_for_normal_product_id(self):
        body = service_fanza_api.get_dvd_product("SSNI-558")
        self.assertEqual(200, body.result.status)
        self.assertEqual(1, body.result.total_count)
        self.assertEqual("ssni558", body.result.items[0].content_id)
        Log.Debug(u"Body returned: {}".format(json.dumps(munchify(body), indent=2, ensure_ascii=False)))
    ...
    def test_get_dvd_product___no_result_for_digital_product_id(self):
        body = service_fanza_api.get_dvd_product("ssni00558")
        self.assertEqual(200, body.result.status)
        self.assertEqual(0, body.result.total_count)
        Log.Debug(u"Body returned: {}".format(json.dumps(munchify(body), indent=2, ensure_ascii=False)))

    def test_get_dvd_product___no_result_for_vr_product_id(self):
        body = service_fanza_api.get_dvd_product("sivr00002")
        self.assertEqual(200, body.result.status)
        self.assertEqual(0, body.result.total_count)
        Log.Debug(u"Body returned: {}".format(json.dumps(munchify(body), indent=2, ensure_ascii=False)))

    def test_get_digital_product(self):
        body = service_fanza_api.get_digital_product("ssni00558")
        self.assertEqual(200, body.result.status)
        self.assertEqual(1, body.result.total_count)
    ...
    def test_get_digital_product___auto_convert_for_normal_product_id(self):
        body = service_fanza_api.get_digital_product("SSNI-558")
        self.assertEqual(200, body.result.status)
        self.assertEqual(1, body.result.total_count)
        self.assertEqual("ssni00558", body.result.items[0].content_id)
        Log.Debug(u"Body returned: {}".format(json.dumps(munchify(body), indent=2, ensure_ascii=False)))
    ...
    def test_get_digital_product___no_result_for_dvd_product_id(self):
        body = service_fanza_api.get_digital_product("ssni558")
        self.assertEqual(200, body.result.status)
        self.assertEqual(0, body.result.total_count)
        Log.Debug(u"Body returned: {}".format(json.dumps(munchify(body), indent=2, ensure_ascii=False)))

    def test_get_digital_product___no_result_for_vr_unconverted_product_id(self):
        body = service_fanza_api.get_digital_product("sivr002")
        self.assertEqual(200, body.result.status)
        self.assertEqual(0, body.result.total_count)
        Log.Debug(u"Body returned: {}".format(json.dumps(munchify(body), indent=2, ensure_ascii=False)))

    def test_get_digital_product___one_result_for_vr_product_id(self):
        body = service_fanza_api.get_digital_product("h_1127vovs00341")
        self.assertEqual(200, body.result.status)
        self.assertEqual(1, body.result.total_count)
    ...
    def test_get_product_description___for_digital_product(self):
        ...
        description = service_fanza_api.get_product_description(url)
    ...
    def test_get_actress(self):
        actress_id = 1031805
        body = service_fanza_api.get_actress(actress_id)
